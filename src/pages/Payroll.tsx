  ...
  const calculateAguinaldo = (employee, period) => {
    ...
    if (employeePayrolls.length > 0) {
      ...
      const salaryCalculations = relevantPayrolls.map((payroll) => {
        ...
        // Fórmula correcta: (depósito + efectivo) - (presentismo + aguinaldo)
        const totalPaid = whiteAmount + informalAmount;
        const excludedConcepts = presentismoAmount + aguinaldoAmount;
        const totalSalary = Math.max(totalPaid - excludedConcepts, 0);
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid gap-6 py-4 lg:grid-cols-2">
              <div className="space-y-4">
                ...
                {/* Pago de Aguinaldo - solo visible en períodos con aguinaldo */}
                {selectedPeriod && isAguinaldoPeriod(selectedPeriod) && calculation?.aguinaldo > 0 && (
                  <div className="col-span-2 p-4 bg-green-50 border border-green-200 rounded-lg space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold text-green-800">Pago del Aguinaldo</h4>
                      <span className="text-sm text-green-700">
                        Total: {formatCurrency(calculation.aguinaldo)}
                      </span>
                    </div>
                    <p className="text-xs text-green-700">
                      Registra cómo se pagará el aguinaldo (en efectivo y/o depósito)
                    </p>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="aguinaldoPagoEfectivo">Aguinaldo Efectivo</Label>
                        <CurrencyInput
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="aguinaldoPagoDeposito">Aguinaldo Depósito</Label>
                        <CurrencyInput
                    ...
                    {(() => {
                      const efectivo = parseFloat(aguinaldoPagoEfectivo) || 0;
                      const deposito = parseFloat(aguinaldoPagoDeposito) || 0;
                      const total = efectivo + deposito;
                      const aguinaldoTotal = calculation.aguinaldo || 0;

                      if (total > 0 && Math.abs(total - aguinaldoTotal) > 0.01) {
                        return (
                          <div className="text-xs text-red-600 font-medium">
                            ⚠️ La suma ({formatCurrency(total)}) no coincide con el aguinaldo total ({formatCurrency(aguinaldoTotal)})
                          </div>
                        );
                      }
