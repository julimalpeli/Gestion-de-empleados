...
vi.mock("@/components/LiquidationsReport", () => ({
  default: () => <div>LiquidationsReportStub</div>,
}));
vi.mock("@/components/SimpleLiquidationsReport", () => ({
  default: () => <div>SimpleLiquidationsReportStub</div>,
}));
vi.mock("@/components/MultipleReceiptsReport", () => ({
  default: () => <div>MultipleReceiptsReportStub</div>,
}));
...
describe("Reports integration (UI)", () => {
  ...
  it("renders Reports header and table headers including Depósito and Efectivo", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Reportes/i }),
    ).toBeInTheDocument();
